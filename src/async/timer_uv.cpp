...
timer_uv::timer_uv( uv_loop_t* lp, std::error_code& err, async::timer::handler handler )
:
m_handler{ handler }
{
...
{
	timer_handle_data* const data = reinterpret_cast< timer_handle_data* >( uv_handle_get_data( reinterpret_cast< uv_handle_t* >( handle ) ) );
