...
TEST_CASE("logicmill::ras [ smoke ] { error handling }")
{
	...
	static const bstream::context<> foo_stream_context({&ras::error_category(), &foo::error_category()});
...
TEST_CASE("logicmill::ras [ smoke ] { bad error category }")
{
	...
	static const bstream::context<> foo_stream_context({&ras::error_category()});
